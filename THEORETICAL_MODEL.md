...
(Winter/Night)│/ /                            \ \_____________________________
              └───────────────────────────────────────────────────────────────► TIME
                 Day 1     Day 2     Day 3        WINTER DROUGHT (Lean Period)
```
...
```
     ...
     Vent Stream B (Carbon Oxidizer)   ────► [       ]       (The Reaction Sparks!)
```
...
1. **Catalytic Primitives (Enzymatic Kinetics):**
   ...
   - `NOT`: Allosteric competitive inhibition or repressor protein binding (e.g., Lac/Trp repressor blocking transcription).
...
3. **Energy & Structural Sinks (Molecular Machines):**
   ...
   - `SEPTUM`: Cleavage furrow structural trigger for cell division (FtsZ contractile ring analogue).
...
2. **Living (Metabolism & Landauer Costs):**  
...
7. **The Arithmetic Core (1-Bit Half-Adder):**  
...
2. **Homology vs. Homoplasy (Convergent Evolution):**
...
3. **Inert Stonewall Impedance Blocks:** Randomly distributed basalt boulders and mineral overhangs take up honeycomb cells, creating sheltered crevices, dead ends, and caves with high physical impedance ($\text{Conductance} = 0$). These natural rock formations protect pioneer organisms from turbulent currents and predator lines.
...
```
  ...
  • Primordial Soup Inoculation Pores
                 │
                 ▼
  100% Identical Historical Run on Any Machine
```
